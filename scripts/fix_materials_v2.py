#!/usr/bin/env python3
"""
ClassicMan.blendのマテリアルを修正してGLBエクスポート（Blender 4.x対応）
"""
            ...
            for slot in obj.material_slots:
                if slot.material:
                    ...
                    if mat.use_nodes:
                        ...
                        # 頭部・顔・肌
                        if any(x in obj_lower or x in mat_lower for x in ['head', 'face', 'body', 'skin', 'nug_base_body', 'arm', 'leg']):
                            # 肌色
                            principled.inputs['Base Color'].default_value = (0.95, 0.8, 0.7, 1.0)
                            principled.inputs['Roughness'].default_value = 0.5
                            principled.inputs['Metallic'].default_value = 0.0
                            # Subsurface設定（Blender 4.x）
                        ...
                        # 目
                        elif any(x in obj_lower or x in mat_lower for x in ['eye', 'cornea']):
                            if 'onuglusion' in mat_lower:
                                # アンビエントオクルージョン用（半透明黒）
                                principled.inputs['Base Color'].default_value = (0.0, 0.0, 0.0, 1.0)
                                principled.inputs['Roughness'].default_value = 0.0
                                principled.inputs['Alpha'].default_value = 0.5
                            elif 'cornea' in mat_lower:
                                # 角膜（透明）
                                principled.inputs['Base Color'].default_value = (1.0, 1.0, 1.0, 1.0)
                                principled.inputs['Roughness'].default_value = 0.0
                                principled.inputs['Transmission Weight'].default_value = 1.0 if 'Transmission Weight' in principled.inputs else 0
                                principled.inputs['Alpha'].default_value = 0.1
                            elif 'tearline' in mat_lower:
                                # 涙線
                                principled.inputs['Base Color'].default_value = (1.0, 1.0, 1.0, 1.0)
                                principled.inputs['Roughness'].default_value = 0.0
                        ...
                        # 爪
                        elif any(x in obj_lower or x in mat_lower for x in ['nail']):
                            principled.inputs['Base Color'].default_value = (0.95, 0.85, 0.8, 1.0)
                            principled.inputs['Roughness'].default_value = 0.2
                            principled.inputs['Metallic'].default_value = 0.0
                            print(f"    -> 爪の色設定")
                            
                        # まつ毛
                        elif any(x in obj_lower or x in mat_lower for x in ['lash', 'eyelash']):
                        ...
                        # その他（グレー）
                            ...
                            if current_color[0] > 0.9 and current_color[1] > 0.9 and current_color[2] > 0.9:
                                principled.inputs['Base Color'].default_value = (0.5, 0.5, 0.5, 1.0)
                                principled.inputs['Roughness'].default_value = 0.5
                                principled.inputs['Metallic'].default_value = 0.0
                                print(f"    -> デフォルト色設定（白から変更）")
...
def main():
    print("ClassicMan マテリアル修正スクリプト v2")
    print("=" * 40)
